...
def test_should_return_highest_bid_for_current_price() -> None:
    auction = AuctionFactory(
        bids=[Bid(id=1, bidder_id=1, amount=get_money("20")), Bid(id=2, bidder_id=2, amount=get_money("15"))]
    )

    assert auction.current_price == get_money("20")
...
def test_should_return_highest_bids_user_id_for_winners_list() -> None:
    auction = AuctionFactory(
        bids=[
            Bid(id=1, bidder_id=1, amount=get_money("101")),
            Bid(id=2, bidder_id=2, amount=get_money("15")),
            Bid(id=3, bidder_id=3, amount=get_money("100")),
        ]
    )
...
def test_should_win_auction_if_is_the_only_bidder_above_starting_price() -> None:
    ...
    auction.place_bid(bidder_id=1, amount=get_money("11"))
...
def test_should_not_be_winning_auction_if_bids_below_starting_price() -> None:
    ...
    auction.place_bid(bidder_id=1, amount=get_money("5"))

    assert auction.winners == []


def test_should_withdraw_the_only_bid() -> None:
    auction = AuctionFactory(bids=[Bid(id=1, bidder_id=1, amount=get_money("50"))])
...
def test_should_not_be_winning_if_bid_lower_than_current_price() -> None:
    auction = AuctionFactory(bids=[Bid(id=1, bidder_id=1, amount=get_money("10.00"))])

    lower_bid_bidder_id = 2
    auction.place_bid(bidder_id=lower_bid_bidder_id, amount=get_money("5.00"))
...
def test_should_not_allow_placing_bids_for_ended_auction(yesterday: datetime) -> None:
    ...
    with pytest.raises(BidOnEndedAuction):
        auction.place_bid(bidder_id=1, amount=auction.current_price + get_money("1.00"))


def test_should_emit_event_upon_overbid() -> None:
    bid_that_will_lose = Bid(id=1, bidder_id=1, amount=get_money("15.00"))
    auction = AuctionFactory(bids=[bid_that_will_lose])

    new_bid_amount = get_money("20.00")
    auction.place_bid(bidder_id=2, amount=new_bid_amount)

    expected_event = BidderHasBeenOverbid(auction.user_id, bid_that_will_lose.bidder_id, new_bid_amount,
                                          auction.title)
...
def test_should_emit_winning_event_if_the_first_offer() -> None:
    auction = AuctionFactory()
    winning_amount = auction.current_price + get_money("1.00")

    auction.place_bid(bidder_id=1, amount=winning_amount)

    assert auction.domain_events == [WinningBidPlaced(auction.user_id, 1, winning_amount, auction.title)]


def test_should_emit_winning_if_overbids() -> None:
    auction = AuctionFactory(bids=[Bid(id=1, bidder_id=1, amount=get_money("15.00"))])
    winning_amount = auction.current_price + get_money("1.00")

    auction.place_bid(bidder_id=2, amount=winning_amount)

    expected_winning_event = WinningBidPlaced(auction.user_id, 2, winning_amount, auction.title)
    expected_overbid_event = BidderHasBeenOverbid(auction.user_id, 1, winning_amount, auction.title)
...
def test_should_emit_auction_ended(yesterday: datetime) -> None:
    auction = AuctionFactory(bids=[Bid(id=1, bidder_id=1, amount=get_money("15.00"))], ends_at=yesterday)

    auction.end_auction()

    expected_event = AuctionEnded(auction.user_id, auction.winners[0], auction.current_price, auction.title)
...
def test_should_emit_event_with_none_winner_if_no_winners(yesterday: datetime) -> None:
    ...
    expected_event = AuctionEnded(auction.user_id, None, auction.current_price, auction.title)
    assert auction.domain_events == [expected_event]
...
def test_EndedAuction_PlacingBid_RaisesException(yesterday: datetime) -> None:
    ...
    with pytest.raises(BidOnEndedAuction):
        auction.place_bid(bidder_id=1, amount=get_money("19.99"))
