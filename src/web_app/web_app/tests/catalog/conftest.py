...
from product_catalog.application.usecases.begin_catalog import DefaultCatalog
from product_catalog.application.usecases.create_catalog import CreateDefaultCatalogUC
...
def default_catalog(container: injector.Injector) -> Catalog:
    with container.get(RequestScope):
        uc = container.get(CreateDefaultCatalogUC)
        dto = DefaultCatalog()
        uc.execute(dto)
